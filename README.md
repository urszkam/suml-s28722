...
- Cloud Build, Cloud Run and Artifact Registry APIs are enabled.
- A Cloud Build trigger runs `cloudbuild.yaml` with a service account that can manage Terraform state, Artifact Registry and Cloud Run.
...
5. Runs Terraform plan and apply with `container_image_tag=${SHORT_SHA}`.
...
3. Build and run the Docker image:
   ```sh
   docker build -t burnout-app:local app
   docker run --rm -p 8080:8080 burnout-app:local
   ```
